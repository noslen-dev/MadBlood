...
public class DataAnalystViewController implements Initializable {
    ...
    public void bloodDonatedByAgeReport(List<Pair<Integer,Integer>> data) {
        ...
        final int total_blood_by_young_adult = data.stream().filter(pair-> pair.first>= 18 && pair.first <=39).mapToInt(pair-> pair.second).sum();
        final int total_blood_by_middle_age_adult = data.stream().filter(pair-> pair.first>= 40 && pair.first <=59).mapToInt(pair-> pair.second).sum();
        final int total_blood_by_old_people = data.stream().filter(pair-> pair.first>= 60 && pair.first <=65).mapToInt(pair-> pair.second).sum();

        if(total_blood_by_young_adult > total_blood_by_middle_age_adult && total_blood_by_young_adult > total_blood_by_old_people)
            ageGroups = "Young adult";
        else if(total_blood_by_middle_age_adult > total_blood_by_young_adult && total_blood_by_middle_age_adult > total_blood_by_old_people)
            ageGroups = "Middle age adult";
        else if(total_blood_by_old_people > total_blood_by_young_adult && total_blood_by_old_people > total_blood_by_middle_age_adult)
            ageGroups = "Old people";
        else
            ageGroups = "Every age group donated the same amount";

        idTextBloodDonatedByAge.setText("The age with most number of donations is: "+ age_with_most_donated_blood.first +
    ...
    public void bloodDonatedByAgeAndGender() throws SQLException {
        ...
        for(Pair<Integer, Integer> pair : maleData){
            seriesMales.getData().add(new XYChart.Data<>(pair.first, pair.second));
        }
        ...
        for(Pair<Integer, Integer> pair : femaleData){
            seriesFemales.getData().add(new XYChart.Data<>(pair.first, pair.second));
        }
    ...
    public void fillbloodDonatedByAgeAndGenderReport(List<List<Pair<Integer,Integer>>> data){
        ...
        idTextBloodDonatedByAgeAndGender.setText("The age with most number of donations is: "+ age_with_most_donated_blood.first +
                ".\nDonating the following amount: " + age_with_most_donated_blood.second +
                ".\nThe age group they belong is: " + ageGroups);

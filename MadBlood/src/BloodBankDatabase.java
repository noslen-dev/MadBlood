...
/**
 * BloodBankDatabase is the class that encapsulates the database used by the blood bank. It posseses methods to
...
public class BloodBankDatabase {
    ...
    /**
     * Constructs a BloodBankDatabase instance.
    ...
    public List<Pair<String,Integer>> getMostDonatedTypesOfBlood() throws SQLException
    {
        ...
        //iterate thru the resultSet to fill the list to return
        while(true)
        {
            query_result.next();
    ...
    /**
     * @return a List in wich each place corresponds to a Pair and the first member of the pair is an age and the
    ...
    public List<List<Pair<Integer, Integer>>> getBloodDonationsByAgeAndGender() throws SQLException{
        ...
        ResultSet female_query_results = database_connection.createStatement().executeQuery(query_to_execute_to_get_male_donations);
     ...
     * @return List in wich each place corresponds to a Pair and the first member of the pair is an age and the
     * second member of the pair is the amount of blood donations made by persons of that age. Take the following
     * return value as an example [ [15, 127], [16, 200], ...]
     * @throws SQLException in case a problem occurs with the database server.
     */
